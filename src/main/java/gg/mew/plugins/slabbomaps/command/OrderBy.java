...
public enum OrderBy implements Comparator<Shop>, Predicate<Shop> {
    ...
    BuyAscending(Comparator.comparingDouble(it -> it.getBuyPrice() / it.getQuantity()), it ->
            it.getBuyPrice() != -1
                    && it.getQuantity() > 0
                    && it.getStock() >= it.getQuantity()),

    BuyDescending(Comparator.<Shop>comparingDouble(it -> it.getBuyPrice() / it.getQuantity()).reversed(), it ->
            it.getBuyPrice() != -1
                    && it.getQuantity() > 0
                    && it.getStock() >= it.getQuantity()),
